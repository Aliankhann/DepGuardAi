...
from .models import (
    ...
    PaginatedUsageResponse, PaginatedUsageSummaryResponse,
)
...
class BackboardClient:
    ...
    async def get_recent_usage(
        ...
        page: int = 1,
        page_size: int = 20,
    ) -> PaginatedUsageSummaryResponse:
        """Get paginated recent billing activity (usage events).
        ...
        Returns:
            PaginatedUsageSummaryResponse with ``data`` (list of UsageEventSummary)
            and ``pagination`` metadata.
        """
        response = await self._make_request(
            "GET",
            "/billing/usage/recent",
            params={"page": page, "page_size": page_size},
        )
        return PaginatedUsageSummaryResponse.model_validate(response.json())
